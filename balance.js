...
const SIM=`
/* ================= BALANCE SIM ================= */
const ALL=['loop','vars','cond','forloop','func','lists','dicts','measure','sterile','maze','slicing','comprehensions','higher'];
function reset(size,extra){
  W.feats=new Set(ALL.concat(extra||[]));
...
/* the seeder only pays if you drop the now-redundant seed() call */
P.reseed=
'n = dish_size() * dish_size()\\n'+
'for i in range(n):\\n'+
'    seed(CULTURE)\\n'+
...
const RUN=[
  ['starter  (no unlocks)','starter',3,4000,null],
  ['row loop','row',3,4000,null],
  ...
  ['spore farm','spore',5,10000,{bacteria:1e6}],
  ...
  ['culture: tight 2-cell','tight',5,8000,null],
  ...
  ['culture blind +replant','snake',5,8000,null,['replant']],
  ['culture snake +preserve','snake',7,12000,null,['preserve']],
  ['culture rewritten +replant','reseed',5,8000,null,['replant']],
